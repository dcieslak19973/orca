...
/** Tim-class load: multiple worktrees × several agent-like streaming terminals. */
...
async function measureStoreInteractionMs(page: Page): Promise<number> {
  return page.evaluate(async () => {
    ...
    // Cheap store mutation + rAF flush — freezes stall this path.
...
export async function seedBulkOpenRemoteSessions(
  page: Page,
  seed: { repoId: string }
): Promise<{ sessions: BulkOpenSession[]; dispose: () => void }> {
 ...
 * the way Tim did ("just opening my remote sessions again").
 */
export async function runBulkOpenFreezeOracle(
  ...
  // Tim trigger: burst open remote sessions (worktree + tab activate).
  const openProbe = await startRendererLagProbe(page)
  const openStarted = Date.now()
  for (const worktreeId of worktreeIds) {
    const tabs = sessions.filter((s) => s.worktreeId === worktreeId)
    for (const tab of tabs) {
      ...
      // No await-settle — Tim-style rapid open.
    }
  }
  ...
  const interactionProbeMs = await measureStoreInteractionMs(page)
  notes.push(`post-storm store interaction=${interactionProbeMs.toFixed(0)}ms`)
