 ...
 *
 * Models Tim/Expand: bulk-open remote sessions under multi-worktree load.
 *
 * Usage:
  ...
  if (createCount > 0) {
    ...
    await mapPool(createJobs, Math.min(parallel, createCount), async (i) => {
      ...
      try {
        ...
        timings.push({ op: 'terminal.create', ms: createdTerm.elapsedMs, ok: true, index: i })
    ...
    for (let offset = 0; offset < switchTargets.length; offset += parallel) {
      ...
      for (const item of batchResults) {
        if (item.ok) {
          ...
          sumSwitchMs += item.ms
          switchCount += 1
          timings.push({ op: 'terminal.switch', handle: item.handle, ms: item.ms, batchWall })
          if (item.ms >= softMs) {
          ...
          timings.push({ op: 'terminal.switch', handle: item.handle, error: item.error })
