...
// Why: center the first diff from a dedicated effect (not DiffViewer's mount
// handler) so it runs after the comment decorator's view zones, which would
// otherwise shift content downward.
export function useDiffViewerFirstDiffAutoScroll({
  diffEditorRef,
  modifiedEditor,
  modelKey,
